...
var PoA = {
  ...
  Check: function (b, rand, stats, val, cBroca, vBroca, pc) {
    var promises = []
          ...
          */
          ...
          if (contracts[i].ex) {
            ...
            for (var j = 0; j < terms.length; j++) {
              ...
              if (b.report.block >= startTerm && b.report.block <= endTerm) { //report block, not probe block
          ...
          // adjust mean and stddev for report times
          var accepted = {}
  ...
  getRange(prand, account, val, stats) {
    ...
    if(lte > 9)lte = 'zzzzzzzzz'
    if(gte.length != lte.length)console.log(range, gte, lte, prand, account, val, stats)
    return [gte, lte]
  },
